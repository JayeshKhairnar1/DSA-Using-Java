...
public class Sorting_Examples {
	...
	/*
	 * void mergesort(int a[],int start,int end) { if(start<end)//at most size 1 {
	 * int mid = (start+end)/2; mergesort(a,start,mid); mergesort(a,mid+1,end);
	 * merger(a,start,mid,end); }
	 * 
	 * }
	 * 
	 * void selection_sort(int a[]) { int min, pos; for (int i = 0; i < a.length;
	 * i++) { // ref min=a[i]; pos = i; for (int j = i + 1; j < a.length; j++) { if
	 * (a[j] < min) { min = a[j]; pos = j; } } //swap a[i] -a[pos] a[pos]=a[i]; a[i]
	 * = min; } }
	 */
